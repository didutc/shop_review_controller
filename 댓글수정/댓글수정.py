...
while True:
    ...
    while True:
        ...
        try:
            ...
            break
    time.sleep(2)
    driver.get("https://healthyflow.co.kr/myshop/board_list.html")
    while True:
        try:                            
            elem = driver.find_element("xpath",'/html/body/div[2]/div[2]/div[3]/div[5]/div[2]/div[2]/ul/li[1]/p').click()
            break
        except:
            pyzard.errorboy()
            time.sleep(2)
    ...
    while True:
        try:                            
            elem = driver.find_element("xpath",'/html/body/div[2]/div[2]/div[3]/div[5]/div[1]/form/div/div/div[4]/div/span[2]/a').click()
            break
        except:
            pyzard.errorboy()
            time.sleep(2)

    value1 ='"'+str(comment_lst[0])+'"'
    print(value1)
    value2 ='"'+str(comment_lst[1])+'"'
    javascript_code = """
    var xpathResult = document.evaluate("/html/body/div[2]/div[2]/div[3]/div[5]/div/form/div/div[2]/table/tbody[1]/tr[1]/td/input", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
    var inputElement = xpathResult.singleNodeValue;

    // 선택한 input 요소가 있는지 확인
    if (inputElement) {
        // input 요소의 값을 빈 문자열로 설정하여 내용 지우기
        inputElement.value = """+str(value1)+""";
    } else {
    ...
    // iframe 내부 document에서 body 요소 선택
    var xpathResult = iframeDocument.evaluate("/html/body", iframeDocument, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
    var bodyElement = xpathResult.singleNodeValue;

    // body 요소가 있는지 확인
    if (bodyElement) {
        // body 요소의 내용을 지우기
        bodyElement.innerHTML = """+str(value2)+""";
    } else {
        console.error("Body element inside iframe not found!");
    }
    """
    ...
    pyzard.report(script_dir+'//댓글컨텐츠.txt', df_from_txt)